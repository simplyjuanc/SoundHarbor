...
import { IDiscogsRelease, parseDiscogsRelease, searchDiscogsAlbum } from "@/lib/utils/discogsUtils";
import { searchSpotifyAlbum } from '@/lib/utils/spotifyUtils'


export const normaliseReleaseData = (discogsAlbum: IDiscogsRelease, spotifyAlbum: Release): Release => {
  const { imgUrl, spotifyUri, releaseDate, barcode } = spotifyAlbum;
  const parsedDiscogsRelease = parseDiscogsRelease(discogsAlbum)

  const data = Object.assign(parsedDiscogsRelease, { imgUrl, spotifyUri, releaseDate, barcode });
