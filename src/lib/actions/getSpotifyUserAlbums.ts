...
import { getTopItems, extractItemIds, getTracksDetails, getSeveralArtistsTracks, getAlbums } from '../utils/spotifyUtils';
...
export async function getSpotifyUserAlbums(accessToken: string): Promise<Release[] | void> {
  try {
    ...
    const userReleases: Release[] = userAlbums.map((album: any) => parseSpotifyAlbumToRelease(album));
